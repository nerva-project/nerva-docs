...
Our cryptocurrency has quite a few unique features:

* **Fair CPU mining, no pools**: XNV is mined with a regular CPU such as the one in your desktop or laptop (as long as it is not too old). This is made possible by CryptoNight-Adaptive, a hashing algorithm designed to keep ASIC, FPGA, GPU and pool mining unfeasible or unprofitable, so every CPU pulls close to the same weight. There are no mining pools by design, which keeps hashrate distributed and the network hard to centralize. The algorithm is actively maintained: our latest hard fork (v13, July 2026) upgraded it to CryptoNight-Adaptive v6. Check out [Mining FAQ](guides/mining/) to learn all about mining XNV.
